...
server_instance: connect_tcp.TCP_Server_Base | None = None
client_instance: connect_tcp.TCP_Client_Base | None = None
...
def _setup_command():
    ...
    server_instance.register_command(command_name="/command", handler=_command_handler, where_to_run="client", run_in_thread=True)  # ty:ignore[unresolved-attribute]
    server_instance.register_command(command_name="/command_done", handler=_command_done_dealing_server, where_to_run="server", run_in_thread=True)  # ty:ignore[unresolved-attribute]


def _setup_client_command():
    print("Setting up client command...")
    client_instance.register_command(command_name="/command", handler=_command_handler_server_setup, where_to_run="server", run_in_thread=True)  # ty:ignore[unresolved-attribute]


def _command_handler(sock, addr, cmd):
    print(f"Received command from {addr}: {cmd}")
    client_class = server_instance.clients  # ty:ignore[unresolved-attribute]
    ...
    for pair in command_client_pair:
        for msg in pair[0]:
            ...
            for client in pair[1]:
                ...
                server_instance.send_message(client_socket=client_socket, message=temp_msg)  # ty:ignore[unresolved-attribute]
...
def _command_handler_server_setup(sock, addr, cmd):
    ...
    if cmd_id == command_total_num:
        ...
        client_instance.file_transfer_client_recv_client_start(message=msg, file_folder_abspath=None)  # ty:ignore[unresolved-attribute]
        client_instance.send_message(client_socket=client_instance.client_socket, message=f'/command_done "{log_filename}" "{log_path}"')  # ty:ignore[unresolved-attribute]
    else:
        pass
...
def _command_done_dealing_server(sock, addr, cmd):
    ...
    received_log_file = os.path.join(server_instance.file_transfer_dir, os.path.basename(log_path))  # ty:ignore[unresolved-attribute]
