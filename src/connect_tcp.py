...
# Protocol constants (file transfer header sizes)
FILENAME_LEN_BYTES = 4
FILESIZE_LEN_BYTES = 8

# Timeout constants (seconds)
TRANSFER_PORT_WAIT_TIMEOUT = 20
CONNECT_RETRY_TIMEOUT = 20
FILE_TRANSFER_START_TIMEOUT = 10
...
class TCP_Server_Base:  # TCP server class
    ...
    def handle_client(self, client_socket, client_address):  # deal with each client
        ...
        if self.is_hand_alloc_port:
            broadcast_clients_port_alloc_range_msg = "/client_alloc_port_range {}".format(self.each_client_port_range)
    ...
    def file_transfer_mode_recv(self, server_file_address, server_file_port, client_socket, client_id, new_save_path, file_name, command):
        ...
        def file_transfer_client_recv(client_id):
            ...
            try:
                name_len_bytes = b""
                while len(name_len_bytes) < FILENAME_LEN_BYTES:
                    chunk = self.recieve_message(client_file_socket, FILENAME_LEN_BYTES - len(name_len_bytes))
                    print(chunk)
                    if not chunk:
                        try:
                ...
                size_bytes = b""
                while len(size_bytes) < FILESIZE_LEN_BYTES:
                    chunk = self.recieve_message(client_file_socket, FILESIZE_LEN_BYTES - len(size_bytes))
                    if not chunk:
                        try:
                            self.send_message(client_file_socket, self.error_sign)
                        except Exception:
    ...
    def diff_multiple_file_diff_multiple_client_transfer_server_recv_client_start(self, message):
        ...
        for each_file_client_pair in file_client_pair_list:
                ...
                try:
                    if os.path.isfile(file):
                        ...
                        if file_folder_transfer_command_message == "":
                            file_folder_transfer_command_message = "/file_folder {}".format(shlex.quote(file))
                        else:
            ...
            for client_addr in each_file_client_pair[0]:
                ...
                if file_folder_transfer_command_message != "":
                    file_folder_transfer_command_message += " {}".format(shlex.quote(str(client_addr)))
            if file_transfer_command_message != "":
        ...
        for client_addr in client_addr_list:
            for transfer_file in transfer_file_list:
                if command_type == "/file":
                    file_transfer_command_message = "/file {} {}".format(shlex.quote(transfer_file), shlex.quote(str(client_addr)))
                    ...
                    folder_transfer_command_message = "/file_folder {} {}".format(shlex.quote(transfer_file), shlex.quote(str(client_addr)))
    ...
    def folder_file_transfer_server_recv_client_start(self, message):
        ...
        def start_file_transfer_with_limit(rel_dir, file, root):
            cmd = "/file_folder {} {} {}".format(shlex.quote(rel_dir), shlex.quote(file), shlex.quote(str(client_addr)))
        ...
        try:
            ...
            while is_find_port:
                ...
                if len(self.file_server_port_list) > 0:
                    ...
                    pass
                waiting_time += 1
                if waiting_time >= TRANSFER_PORT_WAIT_TIMEOUT:
    ...
    def file_transfer_mode(self, filename, server_address, server_port, client_port):
        ...
        while True:
            try:
                ...
                traceback.print_exc()
                if reset_time >= CONNECT_RETRY_TIMEOUT:
                    close_socket()
        ...
        try:
            while True:
                ...
                time.sleep(1)
                waiting_time += 1
                if waiting_time >= FILE_TRANSFER_START_TIMEOUT:
                    try:
                        self.send_message(client_file_socket, self.error_sign)
                    except Exception:
                        traceback.print_exc()
...
class TCP_Client_Base:  # TCP client class
    ...
    def folder_file_transfer_client_recv_client_start(self, message):
        ...
        def send_folder_transfer_command(folder_path, file_name=None, abspath=None):
            folder_transfer_command_message = "/file_folder {}".format(shlex.quote(folder_path))
            if file_name:
                each_file_transfer_command_message = "/file_folder {} {}".format(shlex.quote(folder_path), shlex.quote(file_name))
